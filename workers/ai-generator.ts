...
export interface Env {
  ...
  CACHE?: { get: (key: string) => Promise<string | null>; put: (key: string, value: string, options?: { expirationTtl?: number }) => Promise<void> }; // Optional KV cache for similar content
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === 'POST' && url.pathname === '/api/analyze-complexity') {
      try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/generate') {
      try {
        ...
        const complexity = analyzeContentComplexity(body.markdown);
        const selectedModel = selectOptimalModel(complexity, body.model, env.MODEL);
        ...
        if (env.AI && typeof env.AI.run === 'function') {
          try {
              ...
              try {
                ...
                const raw =
                  typeof result === 'string'
                    ? result
                    : ((result as any)?.response ?? (result as any)?.output_text ?? '');
                ...
                console.warn(`AI generation attempt ${i + 1} failed:`, attemptError);
            ...
            console.warn(`AI generation failed with ${selectedModel.id}, falling back to heuristic:`, e);
          }
        }
...
function selectOptimalModel(complexity: ContentComplexity, requestedModel?: string, envModel?: string): { id: string; type: ModelType } {
  // If a specific model is requested, use it
  if (requestedModel) {
    const modelType = Object.entries(AI_MODELS).find(([_, id]) => id === requestedModel)?.[0] as ModelType;
    if (modelType) {
  ...
  if (envModel) {
    const modelType = Object.entries(AI_MODELS).find(([_, id]) => id === envModel)?.[0] as ModelType;
    if (modelType) {
...
function getSystemPrompt(modelType: ModelType): string {
  ...
  const variations = [
    {
      persona: "You are a viral content creator who makes everything sound exciting and shareable.",
      style: "Use clickbait techniques, power words, and emotional hooks. Make titles irresistible."
    },
    {
      persona: "You are a senior technical architect who speaks to other experts.",
      style: "Use precise technical language, industry jargon, and sophisticated terminology."
    },
    {
      persona: "You are a beginner-friendly educator who makes complex topics accessible.",
      style: "Use simple language, analogies, and step-by-step explanations. Avoid jargon."
    },
    {
      persona: "You are a controversial tech blogger who challenges conventional wisdom.",
      style: "Take strong stances, use provocative language, and spark debate."
    },
    {
      persona: "You are a data-driven analyst who focuses on metrics and results.",
      style: "Use numbers, statistics, specific outcomes, and measurable benefits."
    },
    {
      persona: "You are a storytelling marketer who creates narrative-driven content.",
      style: "Use storytelling techniques, personal experiences, and emotional connections."
    },
    {
      persona: "You are a futuristic tech visionary who focuses on innovation and trends.",
      style: "Use cutting-edge language, future-focused terminology, and trend buzzwords."
    },
    {
      persona: "You are a problem-solving consultant who addresses pain points.",
      style: "Focus on problems, solutions, pain points, and practical applications."
    },
    {
      persona: "You are a minimalist writer who values clarity and simplicity.",
      style: "Use clean, concise language. Avoid fluff and focus on essential information."
    },
    {
      persona: "You are a luxury brand marketer who creates premium positioning.",
      style: "Use sophisticated language, premium positioning, and exclusivity appeals."
    }
  ];
...
function generateVariedUserPrompt(markdown: string): string {
  ...
  const randomPrompt = randomApproach.prompts[Math.floor(Math.random() * randomApproach.prompts.length)];
...
function isSignificantlyDifferent(newResult: Record<string, unknown>, previousResult: Record<string, unknown>): boolean {
...
async function generateContentHash(markdown: string): Promise<string> {
  ...
  const headings = (noCode.match(/^#{1,6}\s+(.+)$/gm) || []).map(h => h.toLowerCase().trim());
