...
async function processContentFile(bucket: R2Bucket, key: string): Promise<CachedContentItem | null> {
  try {
    ...
    const contentType = key.startsWith('portfolio/') ? 'portfolio' : 
                       key.startsWith('blog/') ? 'blog' : 'project'
    const fileName = key.split('/').pop()?.replace('.md', '') || ''
    const category = getCategoryFromTags(attributes.tags || [], fileName)

    return {
      id: fileName,
      title: attributes.title || fileName.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
...
async function rebuildCache(env: Env, trigger: string = 'manual'): Promise<CacheData> {
  ...
  console.log(`📊 Total items: ${allItems.length} (Portfolio: ${portfolioItems.length}, Blog: ${blogItems.length}, Projects: ${projectItems.length})`)
...
function isAuthenticated(request: Request, env: Env): boolean {
  ...
  return authHeader === `Bearer ${env.REBUILD_API_KEY}` || apiKey === env.REBUILD_API_KEY
}
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'GET' && path === '/status') {
        const cacheData = await env.CONTENT_CACHE.get('content-cache', 'json') as CacheData | null
        ...
        return new Response(JSON.stringify({
          ...
          cache: cacheData ? {
            ...
            totalItems: cacheData.metadata?.portfolioCount + cacheData.metadata?.blogCount + cacheData.metadata?.projectCount,
