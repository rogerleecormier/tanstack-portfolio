...
export function SaveAsModal({ open, onOpenChange, initialDir = 'blog', initialName = '', onConfirm }: Props) {
        ...
        <div className="grid gap-4 py-2">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input placeholder="my-post" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
          </div>
          <div className="text-xs text-muted-foreground">Key preview: {key || '—'}</div>
          {error && <div className="text-xs text-destructive">{error}</div>}
        </div>
        <DialogFooter className="flex flex-col items-stretch gap-2">
          {exists && !checking && (
            <div className="text-xs text-amber-600">A file with this name already exists.</div>
          )}
          {exists && (
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={!key || !!error || (exists && !allowOverwrite) || checking} onClick={() => key && onConfirm(key)}>
