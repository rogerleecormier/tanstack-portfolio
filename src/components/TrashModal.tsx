...
interface Item { key: string; size: number; uploaded: string; etag: string }
      ...
      if (code === 'exists') {
        setConfirm({
          ...
          onConfirm: async () => {
            ...
            if (r2.success) { await load(); onRestored?.() }
          }
        })
      }
    }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-2 max-h-[60vh] overflow-y-auto">
            ...
            {!loading && items.length === 0 && <div className="text-sm text-muted-foreground">No trashed items</div>}
            {items.map((it) => (
              <div key={it.key} className="flex items-center justify-between p-2 border rounded">
                <div className="text-sm">
                  <div className="font-mono">{it.key}</div>
                  <div className="text-xs text-muted-foreground">Original: {toOriginal(it.key)}</div>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => restore(it.key)}>Restore</Button>
                  <Button size="sm" variant="outline" onClick={() => { setRestoreAsKey(it.key); setRestoreAsOpen(true); }}>Restore As…</Button>
                </div>
              </div>
            ))}
          </div>
