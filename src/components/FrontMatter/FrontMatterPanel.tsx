...
export function FrontMatterPanel({ markdown, onEdit }: FrontMatterPanelProps) {
  ...
  return (
    <Card className='border border-strategy-gold/20 bg-surface-elevated/30 shadow-sm backdrop-blur-xl dark:border-strategy-gold/20 dark:bg-surface-elevated/30'>
      ...
      <CardContent className='space-y-4 p-4'>
        <div className='space-y-4'>
          {/* Title Section */}
          <div className='rounded-lg border border-strategy-gold/20 bg-surface-elevated/40 p-3 backdrop-blur-md dark:border-strategy-gold/20 dark:bg-surface-elevated/40'>
            <div className='mb-2 flex items-center gap-2'>
              <div className='size-1.5 rounded-full bg-gold-600'></div>
              <span className='text-xs font-semibold uppercase text-strategy-gold dark:text-strategy-gold'>
                Title
              </span>
            </div>
            <div className='truncate font-medium text-gray-200 dark:text-gray-300'>
          ...
          <div className='rounded-lg border border-strategy-gold/20 bg-surface-elevated/40 p-3 backdrop-blur-md dark:border-strategy-gold/20 dark:bg-surface-elevated/40'>
            ...
            <div className='line-clamp-3 text-sm text-gray-400 dark:text-gray-400'>
            ...
            <div className='flex flex-wrap gap-2'>
              {frontmatter.tags?.length ? (
                frontmatter.tags.map(tag => (
                  <Badge
                    key={tag}
                    className='border border-strategy-gold/30 bg-strategy-gold/20 text-strategy-gold dark:border-strategy-gold/30 dark:bg-strategy-gold/10 dark:text-strategy-gold'
                  >
                    {tag}
                  </Badge>
                ))
              ) : (
                <span className='text-sm italic text-gray-400 dark:text-grey-500'>
                  No tags set
                </span>
              )}
            </div>
        ...
        <div className='border-t border-indigo-200/60 pt-3 dark:border-indigo-800/60'>
          <div className='flex items-center gap-2 text-xs text-gray-400 dark:text-grey-500'>
