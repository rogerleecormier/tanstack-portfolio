...
const AI_WORKER_ENDPOINT =
  'https://tanstack-portfolio-ai-contact-analyzer.rcormier.workers.dev';
...
async function analyzeMessageContent(
  ...
  try {
    ...
    if (response.ok) {
      ...
      logger.info(
        'Falling back to keyword-based analysis due to AI worker error'
      );
    ...
    logger.info(
      'Falling back to keyword-based analysis due to request failure'
    );
    return createFallbackAnalysis({
      name,
      email: 'user@example.com', // Default email for fallback
...
const createFallbackAnalysis = (
  ...
  const hasUrgentKeywords =
    /urgent|asap|immediately|emergency|critical|deadline|rush|priority/i.test(
      messageLower + ' ' + subjectLower
    );
  const hasProjectKeywords =
    /project|development|implementation|integration|migration|modernization|strategy|planning|consultation/i.test(
      messageLower
    );
  const hasTechnicalKeywords =
    /technical|architecture|system|platform|api|database|infrastructure|devops|automation/i.test(
      messageLower
    );
  const hasMeetingKeywords =
    /meet|schedule|call|discussion|conversation|consultation|review|planning/i.test(
      messageLower
    );
  const hasSimpleKeywords =
    /quick|simple|basic|question|inquiry|information|help/i.test(messageLower);
  ...
  return {
    ...
    originalMessage:
      formData.message.substring(0, 200) +
...
function generateFollowUpQuestions(
  ...
  const hasTimeline =
    /timeline|deadline|schedule|when|time|duration|timeline/i.test(message);
  const hasTeamSize =
    /team|team size|developers|staff|people|employees|users|organization/i.test(
      message
    );
  const hasBudget =
    /budget|cost|price|investment|funding|financial|expense/i.test(message);
  const hasTechnicalDetails =
    /technology|tech stack|framework|platform|api|database|infrastructure|system/i.test(
      message
    );
  const hasBusinessContext =
    /business|company|industry|sector|market|customers|clients/i.test(message);
  const hasSpecificGoals =
    /goals|objectives|outcomes|results|success|metrics|kpis|targets/i.test(
      message
    );
  const hasStakeholders =
    /stakeholders|decision makers|leadership|management|executives/i.test(
      message
    );
  ...
  if (inquiryType === 'project') {
    ...
    if (!hasBudget)
      questions.push("What's your budget range for this project?");
    ...
    if (!hasBusinessContext)
      questions.push('What industry or business context should I understand?');
    if (!hasSpecificGoals)
    ...
    if (!hasTimeline)
      questions.push("What's the urgency timeline for this request?");
    if (!hasSpecificGoals)
      questions.push('What immediate outcome do you need?');
...
export const analyzeContactForm = async (
  formData: ContactFormData
...
export async function testAIWorker(): Promise<{
  success: boolean;
  error?: string;
  details?: unknown;
}> {
  try {
    ...
    const testData = {
      ...
      message:
        'This is a test message to check if the AI worker is working properly.',
      consent: 'true',
    };
