...
const SUBSCRIPTION_WORKER_ENDPOINT = 'https://tanstack-portfolio-blog-subscription.rcormier.workers.dev'
...
export const updateSubscriptionPreferences = async (
  ...
  try {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to update preferences. Please try again.'
    }
  }
}
...
export const checkSubscriptionStatus = async (email: string): Promise<SubscriptionResponse> => {
  try {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to check status. Please try again.'
    }
  }
}
