...
export interface UpdateWeightGoalRequest
  extends Partial<CreateWeightGoalRequest> {
  id: number;
}
...
export const calculateMedicationProjection = (
  ...
  const medicationTimelineWeeks = Math.ceil(
    totalWeightToLose / medicationWeeklyLoss
  );

  // Calculate impact percentage
  const medicationImpactPercentage =
    ((naturalTimelineWeeks - medicationTimelineWeeks) / naturalTimelineWeeks) *
    100;

  // Projected completion date
  const projectedCompletionDate = new Date();
  projectedCompletionDate.setDate(
    projectedCompletionDate.getDate() + medicationTimelineWeeks * 7
  );
    ...
    return [
      {
        ...
        description:
          'Weekly injection, GLP-1 receptor agonist, typically results in 15-20% weight loss over 68 weeks',
      },
      {
        id: 2,
        name: 'Zepbound',
        ...
        description:
          'Weekly injection, dual GIP/GLP-1 receptor agonist, typically results in 20-25% weight loss over 72 weeks',
      },
      {
        id: 3,
        name: 'Wegovy',
        ...
        typical_duration_weeks: 68,
        description:
          'Higher dose semaglutide specifically for weight loss, same efficacy as Ozempic',
      },
      {
        id: 4,
        name: 'Mounjaro',
        ...
        typical_duration_weeks: 72,
        description:
          'Same medication as Zepbound but marketed for diabetes, same weight loss efficacy',
      },
    ];
  }
};
...
export class UserProfilesAPI {
  static readonly API_BASE_URL =
    'https://healthbridge-enhanced.rcormier.workers.dev/api/v2';

  // User Profile CRUD operations
  static async getUserProfile(userId: string): Promise<UserProfile | null> {
    try {
      const response = await fetch(
        `${UserProfilesAPI.API_BASE_URL}/user/profile?userId=${userId}`,
        {
          method: 'GET',
          headers: {
  ...
  static async updateUserProfile(
    ...
    try {
      ...
      const response = await fetch(
        `${UserProfilesAPI.API_BASE_URL}/user/profile`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  static async getWeightGoal(userId: string): Promise<WeightGoal | null> {
    try {
      const response = await fetch(
        `${UserProfilesAPI.API_BASE_URL}/user/weight-goal?userId=${userId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  static async updateWeightGoal(
    data: UpdateWeightGoalRequest
  ): Promise<WeightGoal> {
    try {
      const response = await fetch(
        `${UserProfilesAPI.API_BASE_URL}/user/weight-goal`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      // Fallback to mock implementation if API fails
      const existingGoal = await UserProfilesAPI.getWeightGoal(
        data.id?.toString() || '1'
      );
      if (!existingGoal) {
  ...
  static async getUserMedications(userId: string): Promise<UserMedication[]> {
    try {
      const response = await fetch(
        `${UserProfilesAPI.API_BASE_URL}/user/medications?userId=${userId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  static async updateUserMedication(
    data: Partial<UserMedication> & { id: string }
  ): Promise<UserMedication> {
    try {
      console.log(
        'Sending medication update to:',
        `${UserProfilesAPI.API_BASE_URL}/user/medication`
      );
