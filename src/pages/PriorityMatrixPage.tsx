...
const PriorityMatrixPage: React.FC = () => {
  ...
  const handleExport = async (type: 'raw' | 'formatted-xlsx' | 'formatted-pdf') => {
    ...
    if (type === 'raw') {
      ...
      const createExcelWithChart = async () => {
        ...
        try {
          ...
          let chartElement = document.querySelector('.relative.mt-8') as HTMLElement;

          // If not found, try to find the chart container div
          if (!chartElement) {
            chartElement = document.querySelector('.priority-matrix-chart')?.parentElement as HTMLElement;
          }

          // If still not found, try the ChartContainer itself
          if (!chartElement) {
            chartElement = document.querySelector('.priority-matrix-chart') as HTMLElement;
          }
          ...
          if (!chartElement) {
            ...
            for (const div of allDivs) {
              if (div.querySelector('.priority-matrix-chart') ||
                  div.querySelector('.recharts-wrapper') ||
                  (div.querySelector('svg') && div.querySelector('[class*="bg-blue-200"]'))) {
                chartElement = div;
                break;
              }
            }
          ...
          if (chartElement) {
            ...
            console.log('Chart captured successfully, image data length:', imageData.length);
      ...
      import('@react-pdf/renderer').then(({ pdf, Document, Page, View, Text, StyleSheet }) => {
        ...
        const ChartVisualization = () => {
          ...
          const scaleX = (value: number) => margin + ((5 - value) / 4) * (chartWidth - 2 * margin);
          const scaleY = (value: number) => margin + ((value - 1) / 4) * (chartHeight - 2 * margin);
        ...
        return (
            <View style={{
              ...
              {/* Grid lines */}
              <View style={{ position: 'absolute', top: margin, left: margin, right: margin, bottom: margin }}>
                {/* Vertical grid lines */}
                {[0, 1, 2, 3, 4].map(i => (
                  <View key={`v-${i}`} style={{
                    position: 'absolute',
              ...
              {matrixData.map((task, index) => {
                ...
                switch (task.quadrant) {
                  case 'Do': color = '#4CAF50'; break;
                  case 'Decide': color = '#FFC107'; break;
                  case 'Delegate': color = '#2196F3'; break;
                  case 'Delete': color = '#F44336'; break;
                }
              ...
              {[1, 2, 3, 4, 5].map(value => (
                <Text key={`urgency-${value}`} style={{
                  position: 'absolute',
                  top: chartHeight - margin + 2,
                  left: margin + ((value - 1) / 4) * (chartWidth - 2 * margin) - 3,
                  fontSize: 7,
                  color: '#666'
                }}>
                  {value}
              ...
              {[1, 2, 3, 4, 5].map(value => (
                <Text key={`importance-${value}`} style={{
                  position: 'absolute',
                  left: margin - 8,
                  top: chartHeight - margin - ((value - 1) / 4) * (chartHeight - 2 * margin) - 3,
                  fontSize: 7,
                  color: '#666'
                }}>
                  {value}
          ...
          <Document>
            <Page size="A4" style={styles.page}>
              <View style={styles.section}>
                ...
                {/* Chart Visualization */}
                <Text style={{ fontSize: 12, marginBottom: 5, textAlign: 'center' }}>
                  Priority Matrix Visualization
                </Text>
                <ChartVisualization />

                {/* Data Table */}
                <Text style={{ fontSize: 12, marginTop: 15, marginBottom: 5, textAlign: 'center' }}>
                  Task Details
                </Text>
                <View style={styles.tableContainer}>
                  <View style={styles.tableHeader}>
        ...
        pdf(<MyDocumentWithChart />).toBlob().then(blob => {
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
        ...
        <div className="relative px-4 py-8 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <P className="text-lg leading-7 text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
              Strategic task prioritization using the Eisenhower Matrix methodology.
              <span className="font-medium text-teal-700 dark:text-teal-300"> Target your priorities </span>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <P className="text-sm text-gray-600 dark:text-gray-400">
                  Add tasks with importance (1-5) and urgency (1-5) scores to generate your priority matrix.
                </P>
                ...
                <div className="space-y-4">
                                    ...
                                    {Array.from({ length: 5 }, (_, i) => i + 1).map(num => (
                                      <SelectItem key={num} value={num.toString()}>
                                        <div className="flex items-center gap-2">
                                          <div className={`w-2 h-2 rounded-full ${num >= 4 ? 'bg-red-500' : num >= 3 ? 'bg-yellow-500' : 'bg-green-500'}`}></div>
                                          {num} - {num >= 4 ? 'Critical' : num >= 3 ? 'High' : num >= 2 ? 'Medium' : 'Low'}
                                        </div>
                                      </SelectItem>
                                    ))}
                                  </SelectContent>
                                ...
                                <Select onValueChange={field.onChange} value={field.value}>
                                  ...
                                  <SelectContent>
                                    {Array.from({ length: 5 }, (_, i) => i + 1).map(num => (
                                      <SelectItem key={num} value={num.toString()}>
                                        <div className="flex items-center gap-2">
                                          <div className={`w-2 h-2 rounded-full ${num >= 4 ? 'bg-red-500' : num >= 3 ? 'bg-orange-500' : 'bg-blue-500'}`}></div>
                                          {num} - {num >= 4 ? 'Immediate' : num >= 3 ? 'Soon' : num >= 2 ? 'Later' : 'Future'}
                                        </div>
                                      </SelectItem>
                                    ))}
                                  </SelectContent>
                  ...
                  <Button
                    type="button"
                    onClick={() => append({ name: '', importance: '3', urgency: '3' })}
            ...
            {matrixData.length > 0 && (
              <div className="mt-12 pt-8 border-t border-teal-200/50 dark:border-teal-700/50">
                <div className="text-center mb-8">
                  ...
                  <P className="text-sm text-gray-600 dark:text-gray-400">
                    Eisenhower Matrix visualization with targeted action quadrants
                  </P>
                </div>
                <>
                  {(() => {
                    ...
                    return (
                      <div className="relative mt-8">
                        ...
                        <ChartContainer config={chartConfig} className="h-[400px] w-full relative z-10 priority-matrix-chart">
