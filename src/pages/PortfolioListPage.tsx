...
const categoryConfig = {
  ...
  'Technology & Operations': {
    ...
    bgColor: 'bg-hunter-50 dark:bg-surface-deep/20',
  ...
  return (
    <div className='min-h-screen bg-surface-deep'>
            ...
            <div className='mb-4 flex items-center justify-center gap-4'>
              ...
              <div>
                <h1 className='text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl'>
                  <span className='text-strategy-gold'>Portfolio & Expertise</span>
                </h1>
      ...
      <div className='mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8'>
        {/* Search and Filters */}
        <div className='mx-auto mb-8 max-w-4xl'>
          <div className='rounded-lg border border-strategy-gold/20 bg-surface-elevated/30 p-6 shadow-lg backdrop-blur-xl dark:border-strategy-gold/20 dark:bg-surface-elevated/30'>
            <div className='flex flex-col gap-4 sm:flex-row'>
              {/* Search Input */}
              <div className='relative flex-1'>
                ...
                <Input
                  ...
                  className='h-12 rounded-lg border-strategy-gold/20 bg-hunter-800/50 pl-12 text-white placeholder:text-grey-500 focus:border-strategy-gold/50 focus:ring-gold-500/20 dark:border-strategy-gold/20 dark:bg-hunter-800/50'
                />
              </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant='outline'
                    className='h-12 rounded-lg border-strategy-gold/20 bg-hunter-800/30 px-6 text-strategy-gold hover:border-strategy-gold/50 hover:bg-hunter-700/50 dark:border-strategy-gold/20 dark:bg-hunter-800/30'
                  >
                    <span className='mr-2'>
                ...
                <DropdownMenuContent className='max-h-60 w-full overflow-y-auto rounded-lg border-strategy-gold/20 bg-surface-elevated/90 shadow-xl backdrop-blur-xl dark:border-strategy-gold/20 dark:bg-surface-elevated/90 sm:w-64'>
                  <DropdownMenuItem
                    onClick={() => setSelectedCategory('all')}
                    className='mx-2 my-1 rounded-lg text-strategy-gold hover:bg-hunter-800/50 dark:hover:bg-hunter-800/50'
                  >
                    All Categories
                  </DropdownMenuItem>
              ...
              >
                ...
                {selectedTags.length > 0 && (
                  <Badge
                    variant='secondary'
                    className='ml-2 bg-gold-600/70 text-hunter-900 dark:bg-strategy-gold dark:text-hunter-950'
                  >
                    {selectedTags.length}
                  </Badge>
                )}
        ...
        <div className='mx-auto mb-8 max-w-4xl'>
          <div className='flex items-center justify-between'>
            <p className='font-medium text-strategy-gold dark:text-strategy-gold'>
              ...
              <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3'>
                {displayedItems.map(item => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className='group relative flex h-full flex-col overflow-hidden rounded-lg border border-strategy-gold/20 bg-surface-elevated/30 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-strategy-gold/40 hover:bg-hunter-800/40 hover:shadow-xl dark:border-strategy-gold/20 dark:bg-surface-elevated/30'
                    >
                      {/* Gold accent on top */}
                      ...
                      <div className='relative flex flex-1 flex-col p-6'>
                        ...
                        <div className='mb-4 flex items-start justify-between'>
                          <Badge
                            variant='secondary'
                            className='border-strategy-gold/30 bg-gold-600/20 text-strategy-gold shadow-sm'
                          >
                        ...
                        {/* Title */}
                        <h3 className='mb-2 text-xl font-bold text-white transition-colors group-hover:text-strategy-gold'>
                          <Link
                            to={`/${item.url}`}
                        ...
                        {/* Description */}
                        <p className='mb-4 line-clamp-3 flex-1 leading-relaxed text-gray-300'>
                          {item.description}
                        </p>
                              ...
                              <>
                                {cleanTags.slice(0, 3).map((tag: string) => (
                                  <Badge
                                    key={tag}
                                    variant='secondary'
                                    className='h-auto border-strategy-gold/30 bg-gold-600/20 px-2 py-1 text-xs text-strategy-gold'
                                  >
                                    <Tag className='mr-1 size-3' />
                                ...
                                {cleanTags.length > 3 && (
                                  <Badge
                                    variant='secondary'
                                    className='border-grey-600/30 bg-grey-700/50 px-2 py-1 text-xs text-gray-300'
                                  >
                                    +{cleanTags.length - 3}
                                  </Badge>
                                )}
                        ...
                        <Link
                          to={`/${item.url}`}
                          className='mt-auto inline-flex w-full items-center justify-center gap-2 rounded-lg bg-gold-600/70 px-4 py-3 font-semibold text-hunter-900 transition-all duration-200 hover:bg-strategy-gold hover:shadow-lg dark:bg-strategy-gold dark:text-hunter-950 dark:hover:bg-gold-400'
                        >
                          View Details
        ...
        <Dialog open={isTagFilterOpen} onOpenChange={setIsTagFilterOpen}>
          <DialogContent className='z-[200] flex max-h-[85vh] max-w-3xl flex-col border-strategy-gold/20 bg-surface-elevated/50 backdrop-blur-xl dark:border-strategy-gold/20 dark:bg-surface-elevated/30'>
            <DialogHeader className='pb-4'>
              <DialogTitle className='flex items-center gap-3 text-xl text-strategy-gold'>
                <div className='flex size-10 items-center justify-center rounded-lg bg-hunter-800/60 ring-1 ring-gold-500/20 backdrop-blur-md'>
